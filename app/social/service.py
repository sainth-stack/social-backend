...
class SocialMediaService:
    ...
    def update_post(
        ...
        if payload.scheduledAt is not None:
            ...
            if post.status == SocialPostStatus.SCHEDULED and new_scheduled:
    ...
    def duplicate_post(self, post: SocialPost, user: User) -> SocialPostOut:
        self.db.refresh(post, attribute_names=["platforms"])
        clone = SocialPost(
            id=uuid.uuid4(),
    ...
    def _validate_ready_to_publish(self, post: SocialPost) -> None:
        ...
        if not publishable:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Only Facebook and Instagram can be published in this release",
            )
